...
class EquipmentUI(QWidget):
    ...
    def slot_clicked(self, slot_name):
        ...
        with open(path, encoding='utf-8') as f:
            ...
            items.sort(key=lambda x: self.tier_order.get(x.get("tier", ""), 99))
    ...
    def refresh_weapon_damage_overlay(self, slot_name):
    ...
    def refresh_all_weapon_damage_overlays(self):
        self.refresh_weapon_damage_overlay("Main-Hand")
        self.refresh_weapon_damage_overlay("Off-Hand")
    ...
    def item_clicked(self, row, column):
        ...
        if self.last_clicked_slot in ["Main-Hand", "Off-Hand"] and is_two_handed:
            ...
            if item.get("type") == "armor":
                ...
                self.refresh_all_weapon_damage_overlays()
        ...
        if label:
            ...
            bg_color = rarity_colors.get(rarity, "#1D1A31")
            label.setStyleSheet(f"border: 3px #454766; background-color:{bg_color}; color: lightgray; font-size: 16px; font-weight: bold;")
    ...
    def recalculate_stats_with_hero_bonus(self):
        combined = {"combat": {}, "survival": {}, "resistance": {}, "magic": {}}

        for slot_name, label in self.slot_labels.items():
            item = getattr(label, "equipped_item", None)
            if not item:
                continue
    ...
    def clear_label_overlays(self, label):
        for child in label.findChildren(QLabel):
            if child != label:
                child.deleteLater()
                
                
    def is_dual_wielding(self):
        main = self.slot_labels.get("Main-Hand")
        off = self.slot_labels.get("Off-Hand")
        main_item = getattr(main, "equipped_item", None)
        off_item = getattr(off, "equipped_item", None)

        if not main_item or not off_item:
            return False

        if main_item.get("handed") != "One-Handed" or off_item.get("handed") != "One-Handed":
            return False

        if main_item.get("weapon_group", "").lower() == "one_handed_shield" or \
        off_item.get("weapon_group", "").lower() == "one_handed_shield":
            return False
